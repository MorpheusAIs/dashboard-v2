-- Morpheus Capital: Active Stakers Count (SEPOLIA TESTNET VERSION)
...
-- IMPORTANT: This version uses sepolia testnet data
-- If your contracts are on mainnet, use active-stakers.sql instead
...
-- Get all stake events (deposits) - USING SEPOLIA TABLE
stake_events AS (
  SELECT 
    p.pool_type,
    p.contract_address,
  ...
  FROM sepolia.logs logs  -- CHANGED: Using sepolia instead of ethereum
...
-- Get all withdrawal events - USING SEPOLIA TABLE
