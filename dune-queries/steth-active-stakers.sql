-- Morpheus Capital: stETH Pool Active Stakers Count
-- This query gets the count of unique active stakers in the stETH deposit pool
-- An "active staker" is someone who currently has a positive balance

WITH 
-- stETH Pool contract
steth_pool AS (
  SELECT 0xFea33A23F97d785236F22693eDca564782ae98d0 as contract_address
),

-- Get all stake events (deposits) for stETH pool
stake_events AS (
  SELECT 
    logs.tx_hash,
    logs.block_time,
  ...
  CROSS JOIN steth_pool sp
  WHERE 
    logs.contract_address = sp.contract_address
...
-- Get all withdrawal events for stETH pool
...
SELECT 
  ...
  SUM(net_balance) / 1e18 as total_steth_staked,
  AVG(net_balance) / 1e18 as avg_stake_per_user,
  
  -- Distribution analysis
  COUNT(CASE WHEN net_balance >= 1e18 THEN 1 END) as stakers_1_plus_steth,
  COUNT(CASE WHEN net_balance >= 10e18 THEN 1 END) as stakers_10_plus_steth,
  COUNT(CASE WHEN net_balance >= 100e18 THEN 1 END) as stakers_100_plus_steth,
  ...
  -- Note: Recently active stakers query removed due to syntax complexity
  -- Can be added back as separate query if needed
...
SELECT 
  CASE 
    WHEN net_balance < 1e18 THEN '< 1 stETH'
    WHEN net_balance < 10e18 THEN '1-10 stETH'  
    WHEN net_balance < 100e18 THEN '10-100 stETH'
    WHEN net_balance < 1000e18 THEN '100-1000 stETH'
    ELSE '1000+ stETH'
  END as stake_range,
