...
- Final source scan for non-native click wrappers identified the CSV upload wrapper and multi-selector focus wrapper; both now carry explicit `data-analytics-label`, `data-analytics-action`, and `data-analytics-destination` metadata for root-tracker coverage without leaking selected file names.
- Focused wrapper QA on rebuilt `http://localhost:3403/builders` confirmed `script.js` and `recorder.js` loaded, then clicked DOM shapes matching the annotated wrappers because the CSV modal is admin/wallet gated and the selector component is not directly reachable without app state. Umami `/api/send` received:
  - `button-click` with `button_text: CSV upload picker`, `action_name: open-csv-upload`, `element_area: section`, and `destination: csv-file-picker`.
  - `button-click` with `button_text: Builder filter selector`, `action_name: focus-multiple-selector`, `element_area: section`, and `destination: multiple-selector-input`.
- Focused semantic-destination QA on fresh local server `http://localhost:3503/builders` verified the root tracker after the destination parser fix. The page loaded `script.js` and `recorder.js` from the Umami host with `200` responses, and injected DOM shapes matching the newly annotated data filters, network filters, subnet network select items, bulk registration network select items, and multi-selector options emitted `button-click` payloads with destinations preserved exactly as `data-filter:Status:all`, `network-filter:Base`, `subnet-network:8453`, `bulk-registration-network:Base`, and `multiple-selector-option:morpheus` with no `null` prefixes.
- Follow-up Oracle remediation annotated the reusable staking table sort headers with `role="button"`, keyboard activation, `action_name: staking-table-sort`, and semantic destinations such as `staking-table-sort:amount-asc`; it also annotated capital lock-period unit `SelectItem`s in both deposit/claim and change-lock flows with `select-claim-lock-period-unit` and `select-change-lock-period-unit` destinations such as `claim-lock-period-unit:months` and `change-lock-period-unit:years`.
- Follow-up capital asset remediation annotated the `UserAssetsTable` dropdown menu actions so `Stake Rewards`, `Withdraw`, `Lock Rewards`, and `Claim Rewards` emit `button-click` payloads with destinations such as `capital-asset-action:stETH:withdraw`.
- Final verification reran `pnpm run build` after the wallet-toast, wrapper annotation, semantic-destination, and privacy-label fixes; it completed successfully with the same pre-existing external Dune/subgraph warnings seen before these analytics changes.
