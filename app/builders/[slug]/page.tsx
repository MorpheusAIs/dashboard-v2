...
import type { BuildersUser, SubnetUser } from "@/app/graphql/types";
...
import {
	type BuilderSubnetUser as StakingBuilderSubnetUser,
	type UseStakingDataProps,
	useStakingData,
} from "@/hooks/use-staking-data";
import {
	type UseStakingContractInteractionsProps,
	useStakingContractInteractions,
} from "@/hooks/useStakingContractInteractions";
import { parseBuilderDescription } from "@/lib/utils";
import { useUrlParams } from "@/lib/utils/url-params";
import type { Builder } from "../builders-data";
...
export default function BuilderPage() {
		...
		if (builders && builders.length > 0) {
			...
			if (subnetIdFromUrl) {
				foundBuilder = builders.find((b) => {
					...
					return (
						b.id === subnetIdFromUrl || b.mainnetProjectId === subnetIdFromUrl
					);
				});
			...
			if (!foundBuilder) {
				...
				const hasNetworkSuffix =
					slug.endsWith("-base-sepolia") ||
					slug.endsWith("-base") ||
					slug.endsWith("-arbitrum");
	...
	// Merge fresh single-builder data (direct Goldsky query) into the builder from context.
	// This ensures on-chain metadata edits (description, website, image) are reflected
	// immediately after the subgraph indexes the update, without waiting for the full
	// BuildersContext cache to be invalidated and refetched.
	const builderWithUpdatedMetrics = useMemo(() => {
		...
		if (singleBuilderData && !isTestnet) {
			return {
				...
				// Prefer on-chain values from the direct Goldsky fetch when present
				description: singleBuilderData.description || builder.description,
				long_description:
					singleBuilderData.description || builder.long_description,
				website: singleBuilderData.website || builder.website,
				image_src: singleBuilderData.image_src || builder.image_src,
				image: singleBuilderData.image || builder.image,
			};
		}
	...
	const contractAddress = useMemo<Address | undefined>(() => {
		const selectedChain = isTestnet
			? testnetChains.baseSepolia
			: chainId === 42161
				? mainnetChains.arbitrum
				: mainnetChains.base;
	...
	const stakingDataHookProps: UseStakingDataProps = useMemo(
		() => ({
			...
			formatEntryFunc: (user: StakingUser) => ({
				...
				timestamp:
					"lastStake" in user && typeof user.lastStake === "string"
						? parseInt(user.lastStake)
						: 0,
				unlockDate:
					("lastStake" in user && typeof user.lastStake === "string"
						? parseInt(user.lastStake)
	...
	const stakingContractHookProps: UseStakingContractInteractionsProps = useMemo(
			...
			onTxSuccess: () => {
				...
				if (!isTestnet && refetchSingleBuilder) {
					...
					queryClient.invalidateQueries({
						queryKey: ["singleBuilder", singleBuilderProjectId],
					});
				...
				setTimeout(async () => {
					...
					if (refetchAllowanceRef.current) {
						try {
							...
							console.error(
								"Error refreshing allowance/approval state:",
								error,
							);
						}
					} else if (
		...
		[
			...
			refreshData,
			isTestnet,
			refetchSingleBuilder,
			singleBuilderProjectId,
			queryClient,
		],
	);
	...
	const onStakeSubmit = async () => {
		...
		if (!subnetId) {
			...
			showAlert(
				"Cannot stake: Subnet ID is missing. This is likely because the builder's mainnet project ID is not set correctly.",
			);
			return;
		}
	...
	const onWithdrawSubmit = async (amountUserWantsToWithdrawStr: string) => {
		...
		if (amountUserWantsToWithdraw > userStakedAmount) {
			showAlert(
				`You cannot withdraw ${amountUserWantsToWithdraw.toFixed(6)} ${tokenSymbol} because you only have ${userStakedAmount.toFixed(6)} ${tokenSymbol} staked.`,
			);
			return;
		}
		...
		if (!rawStakedAmount || rawStakedAmount <= BigInt(0)) {
			console.warn(
				"########## No staked amount available for withdrawal. rawStakedAmount:",
				rawStakedAmount?.toString(),
				"##########",
			);
			showAlert("You have no staked amount to withdraw.");
			return;
		...
		if (amountToWithdrawWei <= BigInt(0)) {
			console.warn(
				"########## Withdrawal amount must be greater than zero. Attempted:",
				amountUserWantsToWithdrawStr,
				"##########",
			);
	...
	const networkInfo = useMemo(() => {
		if (!builder || isTestnet || !networksToDisplay.length)
					...
					<div className="space-y-4">
						...
						<div className="sm:hidden space-y-4">
							<div className="flex items-center gap-4">
								...
								{builderName ? (
									<h1 className="text-xl sm:text-2xl font-bold text-gray-100 flex-1">
										{builderName}
									</h1>
								) : (
	...
	if (buildersError) {
		return (
			<div className="p-8 text-red-500">
				Error loading builder: {buildersError.message}
			</div>
		);
	}
	...
	return (
		<div className="page-container">
			...
			<div className="max-w-5xl mx-auto space-y-8">
				<ProjectHeader
					name={(builderWithUpdatedMetrics ?? builder).name}
					description={parseBuilderDescription(
						(builderWithUpdatedMetrics ?? builder).description,
					)}
					imagePath={(builderWithUpdatedMetrics ?? builder).image_src || ""}
					networks={networksForIconDisplay}
					website={(builderWithUpdatedMetrics ?? builder).website || ""}
					rewardType={builder.reward_types?.[0] || ""}
					backButton={true}
					backPath="/builders"
					builder={builderWithUpdatedMetrics ?? builder}
				...
				<div className={`grid grid-cols-2 lg:grid-cols-4 gap-4`}>
					<MetricCardMinimal
						title="Total Staked"
						value={
							builderWithUpdatedMetrics?.totalStaked ||
							builder?.totalStaked ||
							0
						}
						label="MOR"
					...
					<MetricCardMinimal
						title="Total Claimed"
						value={
							builderWithUpdatedMetrics?.totalClaimed ||
							builder?.totalClaimed ||
							0
						}
						label="MOR"
						autoFormatNumbers={true}
					...
					<MetricCardMinimal
						title="Cumulative stakers"
						value={
							builderWithUpdatedMetrics?.stakingCount ||
				...
				{(() => {
					const isAdmin =
						authUserAddress &&
						builder.admin &&
						authUserAddress.toLowerCase() === builder.admin.toLowerCase();
								...
								warningMessage={
									!isCorrectNetwork()
										...
										: needsApproval &&
											...
											: (() => {
													const isBaseMainnet =
														!isTestnet && networksToDisplay[0] === "Base";
													if (isBaseMainnet) {
														return (
															<>
					...
					const WithdrawalCardWithGlow = (
						<div className="relative">
							<WithdrawalPositionCard
								...
								disableWithdraw={
									!userStakedAmount || timeLeft !== "Unlocked" || isWithdrawing
								}
					...
					if (isAdmin) {
						...
						return (
							<div className="grid grid-cols-1 md:grid-cols-5 gap-8">
								...
								<div className="md:col-span-2">{WithdrawalCardWithGlow}</div>
							</div>
						);
					} else {
					...
					<CardContent>
						{stakingEntriesError ? (
							<div className="text-red-500">
								Error loading staking data: {stakingEntriesError.message}
							</div>
						) : (
							<StakingTable
								entries={stakingEntries}
