...
/**
 * API route to fetch builders projects from Goldsky V4 for Arbitrum network
 * Queries Goldsky V4 directly for real-time data
 */
export async function GET() {
  try {
    const endpoint = SUBGRAPH_ENDPOINTS.GoldskyArbitrumV4;
    
    console.log(`[Goldsky V4 API Arbitrum] Fetching builders projects from ${endpoint}`);
    ...
    console.log(`[Goldsky V4 API Arbitrum] Found ${buildersProjects.length} builders projects`);
    ...
    console.error('[Goldsky V4 API Arbitrum] Error fetching builders projects:', error);
    return NextResponse.json(
      {
