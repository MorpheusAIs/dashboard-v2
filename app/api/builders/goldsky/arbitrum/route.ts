...
export async function GET() {
  try {
    const filePath = path.join(
      ...
      'public',
      'data',
      'goldsky-builders-arbitrum.json'
    );

    // Check if file exists
    if (!fs.existsSync(filePath)) {
      console.warn(
        `[Goldsky API Arbitrum] File not found: ${filePath}. Returning empty response.`
      );
    ...
    console.error('[Goldsky API Arbitrum] Error reading builders data:', error);
    return NextResponse.json(
      {
        buildersProjects: {
          items: [],
